...
def simulate_one_beta_dia_2(par, a_nodes, end_time, return_list=False):
    if len(par) == 4:
        mu_array, alpha_matrix = get_mu_array_alpha_matrix_dia_bp(par[0], par[1:3], len(a_nodes))
    elif len(par) == 6:
        mu_array, alpha_matrix = get_mu_array_alpha_matrix_dia_bp(par[0], par[1:5], len(a_nodes))
    elif len(par) == 8:
        mu_array, alpha_matrix = get_mu_array_alpha_matrix_dia_bp(par[0], par[1:7], len(a_nodes))
...
def simulate_one_beta_off_2(par_ab, par_ba, a_nodes, b_nodes, end_time, return_list=False):
    if len(par_ab) == 4:
