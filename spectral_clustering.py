...
def directed_spectral_cluster(adj,
                              num_classes=2,
                              n_kmeans_init=10,
                              normalize_z=True,
                              verbose=False,
    ...
    cluster_pred = km.fit_predict(R)
    return cluster_pred


def LF_spectral_cluster(adj, num_classes=2, n_kmeans_init=10, normalize_z=True, Laplacian=True, verbose=False,plot_eigenvalues=False,
                        plot_save_path=''):
    """
    Runs spectral clustering on weighted or unweighted adjacency matrix
    ...
    # Compute largest num_classes singular values and vectors of adjacency matrix
    if Laplacian:
        L = np.sqrt(np.sum(adj, 0))
        L[L == 0] = 1
        L = np.diag(1 / L)
        L = np.matmul(np.matmul(L, adj), L)
    else:
        L = adj
    u, s, v = svds(L, k=num_classes)
    v = v.T
    ...
    u = u[:, sorted_ind]
    v = v[:, sorted_ind]
    s = np.diag(np.sqrt(s[sorted_ind]))
    z = np.c_[np.matmul(u, s), np.matmul(v, s)]
    if normalize_z:
...
def spectral_cluster(adj, num_classes=2, n_kmeans_init=10, normalize_z=True, verbose=False, plot_eigenvalues=False, plot_save_path=''):
    """
    Runs spectral clustering on weighted or unweighted adjacency matrix
...
def regularized_spectral_cluster(adj,
                                 num_classes=2,
                                 tau=None,
                                 n_kmeans_init=10,
                                 normalize_z=True):
