...
# defined for 6-alpha model ## for now
def LL_node_pair(i, j, nodes_a, nodes_b, param_ab, events_dict, diff_T_sum_q_dict, T):
    ...
    if diff is not None:
        ...
        if ij_has_events:
            Ri_temp = OneBlockFit.get_Ri_temp_Q(ij_events, e_intertimes_Q, events_dict[(j, i)], betas)  # (#ij_events, Q) array
    ...
    for u in nodes_b:
        ...
        if diff is not None:
            ...
            if ij_has_events:
                Ri_temp = OneBlockFit.get_Ri_temp_Q(ij_events, e_intertimes_Q, events_dict[(i, u)], betas)  # (#ij_events, Q) array
        ...
        if diff is not None:
            ...
            if ij_has_events:
                Ri_temp = OneBlockFit.get_Ri_temp_Q(ij_events, e_intertimes_Q, events_dict[(u, i)], betas)  # (#ij_events, Q) array
...
def cal_new_LL_kernel_sum_move_node(param_tup, T, idx, from_block, to_block, events_dict_bp, N_c, LL_bp, batch = True):
    ...
    if len(param_tup) == 9:
        ...
        print("error - passed parameters tuple is not valid")
        ...
        for b in range(K):
            if a==from_block or a==to_block or b==from_block or b==to_block:
                if len(param_tup) == 9:
                    ...
                    if a == b:
                        LL_bp1[a, b] = OneBlockFit.LL_6_alpha_kernel_sum_dia(par, events_dict_bp1[a][b], T, N_c1[b, 0], M_bp1[a, b])
                    else:
                        LL_bp1[a, b] = OneBlockFit.LL_6_alpha_kernel_sum_off(par, (events_dict_bp1[a][b]), (events_dict_bp1[b][a]), T,
                                                                             N_c1[b, 0], M_bp1[a, b])
                elif len(param_tup) == 7:
                    par = (mu_bp[a, b], alpha_n_bp[a, b], alpha_r_bp[a, b], alpha_br_bp[a, b], alpha_gr_bp[a, b], C_bp[a, b], betas)
                    if a == b:
                        LL_bp1[a, b] = OneBlockFit.LL_4_alpha_kernel_sum_dia(par, events_dict_bp1[a][b], T, N_c1[b, 0], M_bp1[a, b])
                    else:
                        LL_bp1[a, b] = OneBlockFit.LL_4_alpha_kernel_sum_off(par, (events_dict_bp1[a][b]), (events_dict_bp1[b][a]), T,
                                                                             N_c1[b, 0], M_bp1[a, b])
                elif len(param_tup) == 5:
                    par = (mu_bp[a, b], alpha_n_bp[a, b], alpha_r_bp[a, b], C_bp[a, b], betas)
                    if a == b:
                        LL_bp1[a, b] = OneBlockFit.LL_2_alpha_kernel_sum_dia(par, events_dict_bp1[a][b], T, N_c1[b, 0], M_bp1[a, b])
                    else:
                        LL_bp1[a, b] = OneBlockFit.LL_2_alpha_kernel_sum_off(par, (events_dict_bp1[a][b]), (events_dict_bp1[b][a]), T,
                                                                             N_c1[b, 0], M_bp1[a, b])
    if batch:
...
def model_fit_refine_kernel_sum_exact(max_iter, n_alpha, events_dict, N, K, end_time, betas, batch=True, nodes_mem_true = None, verbose=False):
    ...
    # 2) fit model and get parameters, LL
    fit_param, LL_bp, num_events, events_dict_bp, N_c = MBHP.model_fit_kernel_sum(n_alpha, events_dict, nodes_mem_sp,
                                                                                  K, end_time, betas, ref=True)
    print("\tll fit = ", np.sum(LL_bp))
    print("\n")
    # MBHP.print_model_param_kernel_sum(fit_param)

    # no refinement needed if K=1
    if K == 1 :
        sp_tuple = (nodes_mem_sp, fit_param, np.sum(LL_bp), num_events)
        ref_tuple = (nodes_mem_sp, fit_param, np.sum(LL_bp), num_events)
        return sp_tuple, ref_tuple, " No refinement K=1"

    # 3) refinement - max 3 iteration - stop if converged or #blocks decreased
    nodes_mem_ref0 = nodes_mem_sp
    events_dict_bp0 = events_dict_bp
    fit_param0 = fit_param
    N_c0 = N_c
    LL_bp0 = LL_bp
    message = f" ({max_iter}) max iteration reached"
    for ref_iter in range(max_iter):
        print("Refinement Iterantion #", ref_iter + 1)
        if batch:
            nodes_mem_ref1 = nodes_mem_refinement_batch(nodes_mem_ref0, events_dict_bp0, fit_param0, end_time, N_c0, LL_bp0)
        else:
            nodes_mem_ref1 = nodes_mem_refinement_sequen(nodes_mem_ref0, events_dict_bp0, fit_param0, end_time, N_c0, LL_bp0)
        # break if no change in node_membership after refinemnet iteration
        if adjusted_rand_score(nodes_mem_ref0, nodes_mem_ref1) == 1:
            print("--> Break: node membership converged")
            message = f"Break: node membership converged after iteration# {ref_iter+1}"
            break
        # break if number of classes decreased - nodes moving into the biggest block
        classes_ref, n_node_per_class_ref = np.unique(nodes_mem_ref1, return_counts=True)
        if len(classes_ref) < K:
            print("--> Break: number of classes decreased")
            print("nodes/class percentage : ", np.sort(n_node_per_class_ref / np.sum(n_node_per_class_ref)))
            message = f"Break: number of classes decreased after iteration# {ref_iter+1}"
            break
        if verbose:
            # print("nodes/class percentage : ", np.sort(n_node_per_class_ref / np.sum(n_node_per_class_ref)))
            # MBHP.plot_adj(agg_adj, nodes_mem_ref1, K, f"Refinement membership K={K}, iteration={ref_iter}")
            if nodes_mem_true is not None:
                print(f"\tadjusted RI={adjusted_rand_score(nodes_mem_true, nodes_mem_ref1):.3f}")

        # fit model on new refined node membership
        fit_param1, LL_bp1, num_events, events_dict_bp1, N_c1 = MBHP.model_fit_kernel_sum(n_alpha, events_dict, nodes_mem_ref1,
                                                                                     K, end_time, betas, ref=True)
        # break if train log-likelihood decreased
        if np.sum(LL_bp1) < np.sum(LL_bp0):
            message = f"Break: rain ll decreased from {np.sum(LL_bp0):.1f} to {np.sum(LL_bp1):.1f} at iter# {ref_iter + 1}"
            print(f"\n--> {message}")
            break

        print("ll fit ref = ", np.sum(LL_bp1))
        print("\n")
        # MBHP.print_model_param_kernel_sum(fit_param1)
        # set new argument for next loop
        nodes_mem_ref0 = nodes_mem_ref1
    ...
    sp_tuple = (nodes_mem_sp, fit_param, np.sum(LL_bp), num_events)
    ref_tuple = (nodes_mem_ref0, fit_param0, np.sum(LL_bp0), num_events)
    return sp_tuple, ref_tuple, message


#%% Tests
def test_refinement():
    K, N, T_all = 3, 30, 1000  # number of nodes and duration
    n_alpha = 6
    ...
    n_events_all = OneBlockFit.cal_num_events_2(events_dict)
    print("n_events simulated = ", n_events_all)
    agg_adj = MBHP.event_dict_to_aggregated_adjacency(N, events_dict)
    MBHP.plot_adj(agg_adj, nodes_mem_true, K, "True membership")

    MAX_ITER = 15
    model_fit_refine_kernel_sum_exact(MAX_ITER, n_alpha, events_dict, N, K, T_all, betas, batch=True, nodes_mem_true=nodes_mem_true,
                                      verbose=True)
#%% main
    ...
    for T_idx, T in enumerate(T_range):
        for N_idx, N in enumerate(N_range):
            ...
            for it in range(runs):
                ...
                agg_adj = MBHP.event_dict_to_aggregated_adjacency(N, events_dict)
                # MBHP.plot_adj(agg_adj, nodes_mem_true, K, "True membership")
                sp, ref, m = model_fit_refine_kernel_sum_exact(MAX_ITER, n_alpha, events_dict, N, K, T, betas, batch=True,
