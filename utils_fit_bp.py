...
#%% 4-alpha diagonal block pair log-likelihood and fit functions (sum of kernels)
...
#%% 4-alpha off-diagonal block pair log-likelihood and fit functions (sum of kernels)
...
#%% 2-alpha diagonal block pair log-likelihood and fit functions (sum of kernels)
...
def test_simulate_fit_kernel_sum_dia_bp(n_alpha=6):
    ...
    events_list_dia, events_dict_dia = generate_bp.simulate_sum_betas_dia_bp(param_sim, a_nodes, end_time_sim, return_list=True)
    ...
    if n_alpha == 2:
        ll_sum = LL_2_alpha_kernel_sum_dia(param_sim, events_dict_dia, end_time_sim, n_nodes, M)
    elif n_alpha == 4:
        ll_sum = LL_4_alpha_kernel_sum_dia(param_sim, events_dict_dia, end_time_sim, n_nodes, M)
    else:
        ll_sum = LL_6_alpha_kernel_sum_dia(param_sim, events_dict_dia, end_time_sim, n_nodes, M)
    ...
    if n_alpha == 2:
        ...
        print(approx_fprime(p, NLL_2_alpha_kernel_sum_dia, eps, betas, events_dict_dia, end_time_sim, n_nodes, M,
                            T_diff_sums, Ris))
        print("Analytical gradient")
        print(NLL_2_alpha_kernel_sum_dia_jac(p, betas, events_dict_dia, end_time_sim, n_nodes, M, T_diff_sums, Ris))
    elif n_alpha == 4:
        Ris = cal_R_4_alpha_kernel_sum_dia(events_dict_dia, betas)
        print("Finite-difference approximation of the gradient (only for checking)")
        print(approx_fprime(p, NLL_4_alpha_kernel_sum_dia, eps, betas, events_dict_dia, end_time_sim, n_nodes, M,
                            T_diff_sums, Ris))
        print("Analytical gradient")
        print(NLL_4_alpha_kernel_sum_dia_jac(p, betas, events_dict_dia, end_time_sim, n_nodes, M, T_diff_sums, Ris))
    else:
...
def test_simulate_fit_kernel_sum_two_off_bp(n_alpha=6):
    ...
    l, d_ab, d_ba = generate_bp.simulate_sum_betas_off_bp(p_ab, p_ba, a_nodes, b_nodes, end_time_sim, return_list=True)
    print("#nodes_a = ", n_a, ", #nodes_b = ", n_b, ", Duration = ", end_time_sim,
          " #simulated events = ", cal_num_events(d_ab) + cal_num_events(d_ba))

    if n_alpha == 2:
        ll_ab_1 = LL_2_alpha_kernel_sum_off(p_ab, d_ab, d_ba, end_time_sim, n_b, M_ab)
        ll_ba_1 = LL_2_alpha_kernel_sum_off(p_ba, d_ba, d_ab, end_time_sim, n_a, M_ab)
    elif n_alpha == 4:
        ll_ab_1 = LL_4_alpha_kernel_sum_off(p_ab, d_ab, d_ba, end_time_sim, n_b, M_ab)
        ll_ba_1 = LL_4_alpha_kernel_sum_off(p_ba, d_ba, d_ab, end_time_sim, n_a, M_ab)
    else:
    ...
    if n_alpha == 2:
        Ris = cal_R_2_alpha_kernel_sum_off(d_ab, d_ba, betas)
        print("Finite-difference approximation of the gradient (only for checking)")
        print(approx_fprime(p_ab_d, NLL_2_alpha_kernel_sum_off, eps, betas, d_ab, d_ba, end_time_sim, n_b, M_ab,
                            T_diff_sums, T_diff_sums_r, Ris))
        print("Analytical gradient")
        print(NLL_2_alpha_kernel_sum_off_jac(p_ab_d, betas, d_ab, d_ba, end_time_sim, n_b, M_ab, T_diff_sums,
                                             T_diff_sums_r, Ris))
    elif n_alpha == 4:
        Ris = cal_R_4_alpha_kernel_sum_off(d_ab, d_ba, betas)
        print("Finite-difference approximation of the gradient (only for checking)")
        print(approx_fprime(p_ab_d, NLL_4_alpha_kernel_sum_off, eps, betas, d_ab, d_ba, end_time_sim, n_b, M_ab,
                            T_diff_sums, T_diff_sums_r, Ris))
        print("Analytical gradient")
        print(NLL_4_alpha_kernel_sum_off_jac(p_ab_d, betas, d_ab, d_ba, end_time_sim, n_b, M_ab, T_diff_sums,
                                             T_diff_sums_r, Ris))
    else:
        Ris = cal_R_6_alpha_kernel_sum_off(d_ab, d_ba, betas)
        print("Finite-difference approximation of the gradient (only for checking)")
        print(approx_fprime(p_ab_d, NLL_6_alpha_kernel_sum_off, eps, betas, d_ab, d_ba, end_time_sim, n_b, M_ab,
                            T_diff_sums, T_diff_sums_r, Ris))
        print("Analytical gradient")
        print(NLL_6_alpha_kernel_sum_off_jac(p_ab_d, betas, d_ab, d_ba, end_time_sim, n_b, M_ab, T_diff_sums,
                                             T_diff_sums_r, Ris))
    ...
    for i in range(2):
        ...
        if n_alpha == 2:
            start_fit_time = time.time()
            est_p_ab = fit_2_alpha_kernel_sum_off(d_ab, d_ba, end_time_sim, n_b, M_ab, betas)
            end_fit_time = time.time()
            print("Estimated parameters of block pair (a, b):")
            print_param_kernels(est_p_ab)
            ll_ab_est = LL_2_alpha_kernel_sum_off(est_p_ab, d_ab, d_ba, end_time_sim, n_b, M_ab)
            print(f"fit time = {(end_fit_time - start_fit_time):.4f} s, ll_ab = {ll_ab_est}")
            # fit block pair (b, a)
            start_fit_time = time.time()
            est_p_ba = fit_2_alpha_kernel_sum_off(d_ba, d_ab, end_time_sim, n_a, M_ab, betas)
            end_fit_time = time.time()
            print("Estimated parameters of block pair (b, a):")
            print_param_kernels(est_p_ba)
            ll_ba_est = LL_2_alpha_kernel_sum_off(est_p_ba, d_ba, d_ab, end_time_sim, n_a, M_ab)
            print(f"fit time = {(end_fit_time - start_fit_time):.4f} s, ll_ab = {ll_ba_est}")
            print("total ll = ", (ll_ab_est + ll_ba_est))
        elif n_alpha == 4:
            start_fit_time = time.time()
            est_p_ab = fit_4_alpha_kernel_sum_off(d_ab, d_ba, end_time_sim, n_b, M_ab, betas)
            end_fit_time = time.time()
            print("Estimated parameters of block pair (a, b):")
            print_param_kernels(est_p_ab)
            ll_ab_est = LL_4_alpha_kernel_sum_off(est_p_ab, d_ab, d_ba, end_time_sim, n_b, M_ab)
            print(f"fit time = {(end_fit_time - start_fit_time):.4f} s, ll_ab = {ll_ab_est}")
            # fit block pair (b, a)
            start_fit_time = time.time()
            est_p_ba = fit_4_alpha_kernel_sum_off(d_ba, d_ab, end_time_sim, n_a, M_ab, betas)
            end_fit_time = time.time()
            print("Estimated parameters of block pair (b, a):")
            print_param_kernels(est_p_ba)
            ll_ba_est = LL_4_alpha_kernel_sum_off(est_p_ba, d_ba, d_ab, end_time_sim, n_a, M_ab)
            print(f"fit time = {(end_fit_time - start_fit_time):.4f} s, ll_ab = {ll_ba_est}")
            print("total ll = ", (ll_ab_est + ll_ba_est))
        else:
            start_fit_time = time.time()
