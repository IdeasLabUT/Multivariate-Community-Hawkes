...
def load_reality_mining_test_train(remove_nodes_not_in_train=False):
    ...
    """
    train_file_path = join(get_script_path(), 'storage', 'datasets',
                           'reality-mining', 'train_reality.csv')
    test_file_path = join(get_script_path(), 'storage', 'datasets',
                          'reality-mining', 'test_reality.csv')
...
def load_enron_train_test(remove_nodes_not_in_train=False):
    ...
    :return: Three tuples one for each train, test and combined datasets. Each Tuple contains:
             ...
             (list) nodes_not_in_train
    """
    train_file_path = join(get_script_path(), 'storage', 'datasets', 'enron',
                           'train_enron.csv')
    test_file_path = join(get_script_path(), 'storage', 'datasets', 'enron',
                          'test_enron.csv')
...
def load_fb_train_test(remove_nodes_not_in_train=False):
    ...
    """
    train_file_path = join(get_script_path(), 'storage', 'datasets',
                           'facebook-wallposts', 'train_FB_event_mat.csv')
    test_file_path = join(get_script_path(), 'storage', 'datasets',
                          'facebook-wallposts', 'test_FB_event_mat.csv')
...
def load_train_test(train_file_path, test_file_path, combined_duration,
    ...
    train_event_dict, train_duration = load_test_train_data(
        train_file_path, train_node_id_map)
    test_event_dict, test_duration = load_test_train_data(
        test_file_path, test_node_id_map)
    combined_event_dict = load_test_train_combined(train_file_path,
                                                   test_file_path,
                                                   combined_node_id_map)

    return ((train_event_dict, len(train_node_id_map), train_duration),
            (test_event_dict, len(test_node_id_map),
             test_duration), (combined_event_dict, len(combined_node_id_map),
                              combined_duration), nodes_not_in_train)
...
def load_and_combine_nodes_for_test_train(train_path, test_path,
    ...
    if remove_nodes_not_in_train:
        test_nodes_set = test_nodes_set - test_nodes_set.difference(
            train_nodes_set)
...
def load_test_train_data(file, node_id_map, prev_event_dict=None):
    ...
    for i in range(data.shape[0]):
        ...
        if np.int(data[i, 0]) not in node_id_map or np.int(
                data[i, 1]) not in node_id_map:
            continue
...
def load_test_train_combined(train, test, node_id_map):
    ...
    combined_event_dict, _ = load_test_train_data(test, node_id_map,
                                                  combined_event_dict)

    return combined_event_dict


def split_event_list_to_train_test(event_list,
    ...
    train_event_dict, train_duration = load_test_train_data(
        train_event_list, train_node_id_map)
    test_event_dict, test_duration = load_test_train_data(
        test_event_list, test_node_id_map)
    combined_event_dict = load_test_train_combined(train_event_list,
                                                   test_event_list,
                                                   combined_node_id_map)
...
def load_facebook_wall(timestamp_max=1000,
    ...
    if download_file_path is None:
        file_path = join(get_script_path(), 'storage', 'datasets',
                         'facebook-wallposts', 'facebook-wallpost.txt.gz')

    # Downloading the dataset it is not in the storage directory
    if not os.path.exists(file_path):
        print(
            "Downloading Facebook wall-posts dataset from "
    ...
    if largest_connected_component_only:
        ...
        edge_idx_in_largest_cc = np.array(
            [node_id in largest_cc for node_id in data[:, 0]])
    ...
    if timestamp_max is not None:
        ...
        data[:, 2] = (data[:, 2] - min(data[:, 2])) / (
            max(data[:, 2]) - min(data[:, 2])) * timestamp_max

    if train_percentage is not None:
        return split_event_list_to_train_test(
            data, train_percentage=train_percentage)
